...
@implementation KTVideoCapturerParamers
...
@interface KTVideoCapturer () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCapturePhotoCaptureDelegate>
...
@implementation KTVideoCapturer

- (void)dealloc {
    NSLog(@"%s", __func__);
}

- (instancetype)initWithCaptureParam:(KTVideoCapturerParamers *)param error:(NSError *__autoreleasing  _Nullable * _Nullable)error {
    if (self = [super init]) {
        ...
        NSArray *cameras= videoDeviceDiscoverySession.devices;
        // 获取当前方向的摄像头
        ...
        // 设置抓图输出
        self.captureStillImageOutput = [[AVCapturePhotoOutput alloc] init];
        NSDictionary *setDic = @{AVVideoCodecKey:AVVideoCodecTypeJPEG};
        self.outputSetting = [AVCapturePhotoSettings photoSettingsWithFormat:setDic];
        [self.captureStillImageOutput setPhotoSettingsForSceneMonitoring:self.outputSetting];
...
- (void)imageCapture:(void(^)(UIImage *image))completion {
    [self.captureStillImageOutput capturePhotoWithSettings:[AVCapturePhotoSettings photoSettingsWithFormat:@{AVVideoCodecKey:AVVideoCodecTypeJPEG}] delegate:self];
    self.CaptureImageBlock = completion;
}
...
- (void)captureOutput:(AVCapturePhotoOutput *)output didFinishProcessingPhoto:(AVCapturePhoto *)photo error:(NSError *)error {
    ...
    if (self.CaptureImageBlock) {
        self.CaptureImageBlock(image);
    }
}
...
- (void)captureOutput:(AVCaptureOutput *)output didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    if ([self.delegate respondsToSelector:@selector(captureOutput:didOutputSampleBuffer:fromConnection:)]) {
        [self.delegate captureOutput:output
