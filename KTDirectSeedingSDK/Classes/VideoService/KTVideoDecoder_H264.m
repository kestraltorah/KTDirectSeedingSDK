...
@interface KTVideoDecoder_H264 ()

/// sps数据
@property (nonatomic, assign) u_int8_t *sps;
/// sps数据长度
@property (nonatomic, assign) NSInteger spsSize;
/// pps数据
@property (nonatomic, assign) u_int8_t *pps;
/// pps数据长度
@property (nonatomic, assign) NSInteger ppsSize;
/// 解码器句柄
@property (nonatomic, assign) VTDecompressionSessionRef decoderSeesionRef;
/// 视频解码信息句柄
@property (nonatomic, assign) CMVideoFormatDescriptionRef decoderFormatDesRef;

@end

@implementation KTVideoDecoder_H264
...
static void decodeOutputDataCallback(void *decompressionOutputRefCon, void *sourceFrameRefCon, OSStatus status, VTDecodeInfoFlags infoFlags, CVImageBufferRef pixelBuffer, CMTime presentationTimeStamp, CMTime presentationDuration) {
    ...
    KTVideoDecoder_H264 *decoder = (__bridge KTVideoDecoder_H264 *)decompressionOutputRefCon;
...
-(BOOL)initH264Decoder {
    ...
    // 根据sps pps创建解码视频参数
    OSStatus status = CMVideoFormatDescriptionCreateFromH264ParameterSets(kCFAllocatorDefault, 2, parameterSetPointers, parameterSetSizes, 4, &_decoderFormatDesRef);
    if(status != noErr) {
        NSLog(@"H264Decoder::CMVideoFormatDescriptionCreateFromH264ParameterSets failed status = %d", (int)status);
    }

    // 从sps pps中获取解码视频的宽高信息
    CMVideoDimensions dimensions = CMVideoFormatDescriptionGetDimensions(_decoderFormatDesRef);
    ...
    // 创建解码器
    status = VTDecompressionSessionCreate(kCFAllocatorDefault, _decoderFormatDesRef, NULL, (__bridge CFDictionaryRef)destinationPixelBufferAttributes, &callBackRecord, &_decoderSeesionRef);
    // 解码线程数量
    VTSessionSetProperty(_decoderSeesionRef, kVTDecompressionPropertyKey_ThreadCount, (__bridge CFTypeRef)@(1));
    // 是否实时解码
    VTSessionSetProperty(_decoderSeesionRef, kVTDecompressionPropertyKey_RealTime, kCFBooleanTrue);
...
-(void)decode:(uint8_t *)frame withSize:(uint32_t)frameSize {
    ...
    status = CMSampleBufferCreateReady(kCFAllocatorDefault, blockBuffer, _decoderFormatDesRef , 1, 0, NULL, 1, sampleSizeArray, &sampleBuffer);
    ...
    OSStatus decodeStatus = VTDecompressionSessionDecodeFrame(_decoderSeesionRef, sampleBuffer, flags, NULL, &flagOut);
